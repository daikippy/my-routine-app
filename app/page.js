...
export default function Home() {
  ...
  // --- Core Save Logic (Auto Save) ---
  const saveToFirebase = async (updatedData = {}) => {
    if (!user) return;
    
    // 現在の最新ステートをマージ（引数で渡された場合はそれを優先）
  ...
  const handleSetTheme = (idx) => {
    setThemeIndex(idx);
    saveToFirebase({ themeIndex: idx });
  };

  const handleSetChar = (idx) => {
    setCharIndex(idx);
    saveToFirebase({ charIndex: idx });
  };
          ...
          {activeTab === "main" ? (
            <>
              <div className="bg-white/5 p-6 sm:p-8 rounded-[3.5rem] border border-white/10 mb-6 flex flex-col items-center relative shadow-2xl">
                ...
                <div className={`w-32 h-32 rounded-full ${currentChar.color} shadow-2xl flex items-center justify-center animate-bounce-rich`}>
              ...
              {["morning", "afternoon", "night"].map(time => (
                <div key={time} className="bg-white/5 p-6 rounded-[2.5rem] mb-6 border border-white/10 shadow-xl group">
                  ...
                  <div className="space-y-4">
                    {tasks[time].map((task, index) => (
                      <div key={index} className="flex items-center group/item transition-all">
                        <button onClick={() => toggleCheck(time + task)} className={`w-6 h-6 mr-3 rounded-lg border-2 border-white/10 flex items-center justify-center ${checks[time + task] ? "bg-emerald-500 border-none shadow-lg shadow-emerald-500/30" : "bg-black/20"}`}>
                  ...
                  <div className="flex mt-6 gap-2">
                    ...
                    <button onClick={() => addTask(time)} className="bg-white text-black px-5 rounded-xl font-black text-[10px]">ADD</button>
                  </div>
                </div>
              ))}
              <button onClick={() => saveToFirebase()} className={`w-full py-5 bg-gradient-to-r ${currentTheme.accent} text-gray-950 rounded-[2.5rem] font-black shadow-2xl active:scale-95 transition-all uppercase tracking-[0.2em] text-sm`}>Auto Saved ✅</button>
            </>
          ) : (
            ...
            <div className="space-y-12">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {CHARACTERS.map((c, i) => (
                    <button key={i} onClick={() => handleSetChar(i)} className={`p-4 rounded-[2rem] border-2 transition-all flex flex-col items-center ${charIndex === i ? 'border-white bg-white/10' : 'border-transparent opacity-30'}`}>
              ...
              <section>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {THEMES.map((t, i) => <button key={i} onClick={() => handleSetTheme(i)} className={`w-10 h-10 rounded-full border-2 transition-all ${themeIndex === i ? 'border-white scale-110' : 'border-transparent'}`} style={{ backgroundColor: t.color }}></button>)}
                </div>
              </section>
